...
$input$dark maroon bags contain 2 striped silver bags, 4 mirrored maroon bags, 5 shiny gold bags, 1 dotted gold bag.
...
vibrant olive bags contain 3 muted turquoise bags, 5 wavy blue bags, 1 dotted silver bag, 5 striped tan bags.$input$, '\n') WITH ORDINALITY as t(rules, id)
)
,parsed as(
select
       rules::text,
       id,
       (regexp_match(rules,'(.*) bags contain'))[1] as outer_bag,
--        (regexp_match(rules,'bags contain (\d+ .+?) bags?[\.\,]')) as inner_bags,
--        (regexp_match(rules,'bags contain (\d+ .+?) bags?[\.\,]'))[2] as inner_bags,
       (regexp_split_to_table((regexp_match(rules,'bags contain (\d+ .+?)[\.\,]'))[1],'\s*,\s*')) as inner_bag_raw
from inputs i
)
,reparsed as (
select *,
       ((regexp_match(inner_bag_raw,'\d+'))[1])::bigint as inside_count,
       (regexp_match(inner_bag_raw,'\d+ (.*) bag'))[1] as inside_bag
from parsed
)
,bags_on_bags(inside_bag,outer_bag) as(
select inside_bag, outer_bag
from reparsed r
where inside_bag = 'shiny gold'
union all
select bob.outer_bag, r2.outer_bag
from bags_on_bags bob
inner join reparsed r2
    on r2.inside_bag = bob.outer_bag
)
-- select count(distinct outer_bag)
-- from bags_on_bags

,bags_in_bags(inside_bag,outer_bag,bag_count,iter) as(
select inside_bag, outer_bag, inside_count, 1
from reparsed r
where outer_bag = 'shiny gold'
union all
select r2.inside_bag, bib.inside_bag, bib.bag_count * r2.inside_count, iter + 1
from bags_in_bags bib
inner join reparsed r2
    on r2.outer_bag = bib.inside_bag
)
,part2 as (
select inside_bag,outer_bag,sum(bag_count) as group_bag_counts
from bags_in_bags
group by 1,2
)
select 'part1'::text as part,
       count(distinct outer_bag) as answer
from bags_on_bags
union all
select 'part2'::text,
       sum(group_bag_counts)
from part2
;
