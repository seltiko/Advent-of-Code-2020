...
eyr:2021
hcl:#602927 iyr:2014
pid:274974402 hgt:183cm', '\n\n') WITH ORDINALITY as t(passports, id)
)
,important_fields as(
    select 'byr'::text as field_name
    union select 'iyr'
    union select 'eyr'
    union select 'hgt'
    union select 'hcl'
...
,parsed as(
select regexp_split_to_table(passports,'\s') as fields, id
from inputs i
)
/*select *,
       ((regexp_match(p.fields,':([\d]{4})$'))[1])::int,
       (regexp_match(p.fields,':([a-z]{3})$'))[1],
    case when if2.field_name = 'byr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 1920 and 2002 then 1
           when if2.field_name = 'iyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2010 and 2020 then 1
           when if2.field_name = 'eyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2020 and 2030 then 1
           when if2.field_name = 'hgt' then case
               when p.fields ~ ':\d+cm$' and nullif(regexp_replace(p.fields,'[a-z:]','','g'),'')::int between 150 and 193 then 1
               when p.fields ~ ':\d+in$' and nullif(regexp_replace(p.fields,'[A-z:]','','g'),'')::int between 59 and 76 then 1 end
           when if2.field_name = 'hcl' and (regexp_match(p.fields,':#[a-f\d]{6}$'))[1] is not null then 1
           when if2.field_name = 'ecl' and (regexp_match(p.fields,':([a-z]{3})$'))[1] in ('amb','blu','brn','gry','grn','hzl','oth') then 1
           when if2.field_name = 'pid' and char_length(nullif(regexp_replace(p.fields,'[a-z:]','','g'),'')) = 9 then 1
           else 0 end,
    sum(case when if2.field_name = 'byr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 1920 and 2002 then 1
           when if2.field_name = 'iyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2010 and 2020 then 1
           when if2.field_name = 'eyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2020 and 2030 then 1
           when if2.field_name = 'hgt' then case
               when p.fields ~ ':\d+cm$' and nullif(regexp_replace(p.fields,'[a-z:]','','g'),'')::int between 150 and 193 then 1
           ...
           when if2.field_name = 'ecl' and (regexp_match(p.fields,':([a-z]{3})$'))[1] in ('amb','blu','brn','gry','grn','hzl','oth') then 1
           when if2.field_name = 'pid' and char_length(nullif(regexp_replace(p.fields,'[a-z:]','','g'),'')) = 9 then 1
           else 0 end) over(partition by id)
from parsed p
inner join important_fields if2
    on if2.field_name = split_part(p.fields,':',1)
-- where p.fields ~ 'hcl'
;*/
,checks as(
select id,
       count(*) as valid_fields_provided,
       sum(case when if2.field_name = 'byr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 1920 and 2002 then 1
           when if2.field_name = 'iyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2010 and 2020 then 1
           when if2.field_name = 'eyr' and ((regexp_match(p.fields,':([\d]{4})$'))[1])::int between 2020 and 2030 then 1
           when if2.field_name = 'hgt' then case
...
select count(*) as part1,
       sum(case when valid_data = 7 then 1 else 0 end) as valid_data
from checks c
;
